...
q5 <- plot_histogram(whiteWine$chlorides, 0.001) +
...
quality~prop_free.sulfur.dioxide:<br>
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hold'}
...
p1 <- plot_boxplot(whiteWine$alcohol) +
  labs(y = "Alcohol")
p2 <- plot_boxplot(whiteWine$chlorides) +
  labs(y = "Chlorides") +
  ylim(c(0, 0.25))
...
From the boxplots above we can see patterns of slight correlations between quality and alcohol, chlorides, density, and free SO2 proportion. We'll further investigate the correlations one by one. <br><br><br>
...
It's clearly shown that high quality wines tend to have higher alcohol content and poorer quality wines to have lower alcohol content.<br><br><br>

####Chlorides (Eliminated top 2% chloride levels)
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hold'}
ggplot(aes(x = chlorides, fill = quality.bucket, color = quality.bucket), 
       data = subset(whiteWine, chlorides < quantile(chlorides, probs = 0.98))) +
  geom_density(alpha = 0.4)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
chlorides_by_quality <- summarise(quality_groups, chlorides_mean = mean(chlorides), chlorides_median = median(chlorides), n = n())
arrange(chlorides_by_quality, quality.bucket)
```

After adjusting for the long tail, the difference among the three quality groups is still not very clear to see. Although the difference in chloride level is minimal, we can see that higher content of chlorides tend to exist among lower quality wines. <br><br><br>
...
Although the conditional mean/median comparison indicates that the difference among the groups is minimal, the density plot shows that within a small range of density levels, poorer quality wines tend to have higher density level and better quality wines tend to have lower.<br><br><br>
...
<br>
Among the models above, model 5 captured the most variance (adj. R^2 = 0.232) in the dataset and has the lowest BIC among all. We'll take a look at the distribution of residuals of model 5. <br><br>
